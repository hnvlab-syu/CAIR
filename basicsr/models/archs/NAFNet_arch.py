...
class NAFGroup(nn.Module):
    def __init__(self, n_feats, num_nb=2):
        super().__init__()
        self.body = nn.Sequential(*[NAFBlock(n_feats) for _ in range(num_nb)])
        self.tail = nn.Conv2d(
            in_channels=n_feats,
...
class ColorMISONet(nn.Module):
    def __init__(
        ...
        self.color_attention3 = ColorAttention(
            in_channel=img_channel, num_feat=width * 2
        )
        self.color_attention4 = ColorAttention(
            in_channel=img_channel, num_feat=width * 4
        )
        self.upsample = Upsample(scale=2, num_feat=width)
        self.conv1x1 = nn.Conv2d(
            in_channels=width,
...
class DropPath(nn.Module):
    ...
    def forward(self, *feats):
        ...
        if self.training and factor != 1.0:
            new_feats = tuple(
                [x + factor * (new_x - x) for x, new_x in zip(feats, new_feats)]
            )
        return new_feats
...
class NAFL(nn.Module):
    ...
    def __init__(
        ...
        self.body = MySequential(
            *[
                DropPath(drop_path_rate, NAFBlock(width, drop_out_rate=drop_out_rate))
        ...
        self.conv1 = nn.Conv2d(
            in_channels=num_blks * width, out_channels=width, kernel_size=1
        )
