...
class ImageRestorationModel(BaseModel):
    ...
    def grids(self):
        ...
        crop_size_h, crop_size_w = (
            crop_size_h // self.scale * self.scale,
            crop_size_w // self.scale * self.scale,
        )
        ...
        step_j = (
            crop_size_w
            if num_col == 1
            else math.ceil((w - crop_size_w) / (num_col - 1) - 1e-8)
        )
        step_i = (
            crop_size_h
            if num_row == 1
            else math.ceil((h - crop_size_h) / (num_row - 1) - 1e-8)
        )
