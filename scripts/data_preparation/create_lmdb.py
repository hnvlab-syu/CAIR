...
def create_lmdb_for_iffi():
    """Create lmdb files for IFFI dataset.
    ...
    """
    folder_path = 'datasets/IFFI/train/input'
    lmdb_path = 'datasets/IFFI/train/input.lmdb'
    img_path_list, keys = prepare_keys_iffi(folder_path)
    make_lmdb_from_imgs(folder_path, lmdb_path, img_path_list, keys)

    folder_path = 'datasets/IFFI/train/target'
    lmdb_path = 'datasets/IFFI/train/target.lmdb'
    img_path_list, keys = prepare_keys_iffi(folder_path)
    make_lmdb_from_imgs(folder_path, lmdb_path, img_path_list, keys)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--dataset',
        type=str,
        help=("Options: 'IFFI', 'REDS', 'Vimeo90K' You may need to modify the corresponding configurations in codes."))
    args = parser.parse_args()
    dataset = args.dataset.lower()
    if dataset == 'iffi':
        create_lmdb_for_iffi()
    if dataset == 'df2k':
